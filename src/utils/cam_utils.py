...
class ModelLayerDetector:
    ...
    def get_recommended_layers(model) -> Dict[str, str]:
        ...
        # Check actual layer names in model
        layer_names = [name for name, _ in model.named_modules()]

        # EfficientNetV2 (timm) uses 'conv_head' as final conv layer
        # EfficientNet-B0 (torchvision) uses 'features.8'
        if arch == 'efficientnet':
            if 'conv_head' in layer_names:
                recommended_layer = 'conv_head'  # timm EfficientNetV2
            elif any('blocks.5' in name for name in layer_names):
                recommended_layer = 'blocks.5'  # timm EfficientNetV2 last block
            else:
                recommended_layer = 'features.8'  # torchvision EfficientNet
        else:
            layer_map = {
                'mobilenet': 'features.18',    # MobileNetV2最后卷积层
                'resnet': 'layer4',            # ResNet最后残差块
                'unknown': 'conv_head'         # 默认尝试timm格式
            }
            recommended_layer = layer_map.get(arch, 'conv_head')
